...
class LiveLinkDataTrack(FrameProcessor):
    ...
    def __init__(self, daily_transport=None, use_udp=True, udp_ip="127.0.0.1", udp_port=11111):
        ...
        log_file = self._log_dir / f"livelink_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        if self._use_udp:
            ...
            self._file_logger.info(f"LiveLinkDataTrack initialisé - UDP {self._udp_ip}:{self._udp_port}")
        else:
            self._file_logger.info(f"LiveLinkDataTrack initialisé - Daily datatrack")
    ...
    async def process_frame(self, frame, direction=FrameDirection.DOWNSTREAM):
        ...
        if isinstance(frame, BlendshapeFrame):
            ...
            try:
                if self._use_udp:
                    # Envoi via UDP
                    self._socket.sendto(encoded_data, (self._udp_ip, self._udp_port))
                elif self._tx:
                    # Envoi via Daily datatrack (format simplifié JSON)
                    msg = json.dumps({
                        "blendshapes": frame.values,
                ...
                if frame.frame_idx % 10 == 0:  # Log uniquement périodiquement
                    self._file_logger.info(f"LiveLink: envoi frame blendshapes {frame.frame_idx}")
        ...
        # Traiter les visèmes (compatibilité)
            ...
            try:
                ...
                self._file_logger.info(f"LiveLink: envoi visème {viseme_id} à t={time_ms}ms")
