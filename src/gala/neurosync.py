...
class NeuroSyncClient:
    ...
    async def send_audio(self, audio_data: bytes, sample_rate: int = 16000, channels: int = 1) -> bool:
        """
        ...
        try:
            ...
            if not audio_data or len(audio_data) < 100:
                print(f"⚠️ NeuroSyncClient: données audio trop petites ({len(audio_data) if audio_data else 0} octets)")
                return False
                
            # Log des 10 premiers octets pour débogage
            print(f"🔍 Envoi de {len(audio_data)} octets. Début des données: {audio_data[:10].hex()}")
            ...
            async with self._session.post(
                ...
                try:
                    ...
                    print(f"✅ Réponse reçue de l'API, taille: {len(str(result))} caractères")
                ...
                if "blendshapes" in result and isinstance(result["blendshapes"], list):
                    ...
                    print(f"✅ NeuroSync: reçu {len(blendshapes)} frames de blendshapes en {proc_time:.3f}s")
                    return True
                else:
                    print(f"⚠️ NeuroSync: pas de blendshapes dans la réponse. Contenu: {list(result.keys())}")
                    return False
            ...
            print(f"❌ NeuroSyncClient timeout")
            return False
